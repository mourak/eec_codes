...
void AscanCscan::ascanCallback(const x_eureyecase_messages_generation::OCTRaw::ConstPtr& msg)
{
  ...
  //Rotation along the robot axis : roughly estimate postition at 90deg =  1.65187257817 =y?
  //Rotation perpendicular to the the robot axis : roughly estimate postition at 90deg =  -1.50129802944 =z?

  // created angle variables theta and alfa replacing y and z, corrected for their respective zero position

  float theta = y - 1.67693837903;  // check that the value is correct after each calibration

  //- 1.687;
  float phi = z;
  //- 0.0722;
  float radius = -x*10 ; // all distances are in decimeter
  // the zero position to reach is -0.0502103063185
  float lengthAir = 0.0001*10; // change every time the distance to adapt the reference arm of the OCT system


  std::cout << "theta :: "<< theta << std::endl;
  std::cout << "phi :: "<<phi << std::endl;

  // modified for the Robot end effector
  //-------------------------------------
  double x_0 = (radius + lengthAir) * sin(theta) * cos(phi);
  double y_0 = (radius + lengthAir) * sin(phi);
  double z_0 = (radius + lengthAir) * cos(theta) * cos(phi);
  double deltaL = 0.039/1024;
  //-------------------------------------
  ...
  if (x!= prev_x  || y!=prev_y || z!=prev_z){
      ...
      // Application on Robot encoder acquisition
      //-------------------------------------------------------------------
      pcl::PointXYZI pointI;
      pointI.y = 0;
      for (int i=0; i<ascan_filtred.size(); i++){
          pointI.x = x_0 + (i * deltaL * sin(theta) * cos(phi));
          pointI.y = y_0 + (i * deltaL * sin(phi));
          pointI.z = z_0 + (i * deltaL * cos(theta) * cos(phi));
...
void AscanCscan::Cmd_Ros_Callback(const std_msgs::String::ConstPtr& msg)
{
    ...
    *   ---------------------------------------------------------- **/
    sub_XYZ = nh.subscribe("/Robot/RTP_Pos", 10, &AscanCscan::XYZCallback, &Read);
